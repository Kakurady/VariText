...
vector getGridOffset(integer index){
   ...
   return <(col + 0.5)/columns - 0.5, -(row+0.5)/rows + 0.49, 0>;
}
...
do_layout(string input){
    ...
    for (i = 0, j=0; i< prims && j < len; i++, j+=5){
        ...
        if (x_side > space_before_l + x_l + x_m + x_n_left || x_side > x_n_right + x_o + x_p + space_after_p){
            llOwnerSay("Not enough space in texture avaliable to pad out prim face:" +llGetSubString(input, j, j+3)+ " "+(string)[x_side, " ", space_before_l + x_l + x_m + x_n_left, " ", x_n_right + x_o + x_p + space_after_p]);
        }
        ...
        float x_hollow_gap_left = cut_end * x_side * hollow;
